...
def get_content(img, ocr, x_box=15, y_box=10, num_box=0.5, save_folder=r'c:\表格'):
    try:
        if isinstance(ocr, PPStructure):
            ...
            result = ocr(img, cls=True)
            if isinstance(result, tuple) and len(result) == 3:
                dt_boxes, rec_res, _ = result  # 忽略第三个返回值
            elif isinstance(result, tuple) and len(result) == 2:
                dt_boxes, rec_res = result
            else:
                raise ValueError(f"Unexpected number of values returned from ocr function: {len(result)}")
            dt_num = len(dt_boxes)
            rec_res_final = []
            for dno in range(dt_num):
                ...
                rec_res_final.append({
                    ...
                    'text_region': dt_boxes[dno].astype(int).tolist()  # 将 np.int 替换为 int
                })
